...
def run_quora_mission(sql_db, terms, scraper):
    quora_querys = []
    for term in terms:
    ...
    mission = Mission(sql_db, "quora", quora_querys, quora_parser, scraper)
    mission.run(delay=0.2, thread_num=4)


def run_pcMag_mission(sql_db, terms, scraper):
    pcMag_queries = []
    for term in terms:
        scrap_query = ScrapQuery([term], template="definition of \"{}\"", domain="site: pcmag.com/encyclopedia/")
        pcMag_queries.append(scrap_query)
    pcMag_parser = PcMagParser()
    mission = Mission(sql_db, "pcMag", pcMag_queries, pcMag_parser, scraper)
    mission.run(delay=0, thread_num=4)


def run_regularParse_mission(sql_db, terms, scraper):
    regular_terms = []
    for term in terms:
        scrap_query = ScrapQuery([term], template="definition of \"{}\"")
        regular_terms.append(scrap_query)
    regular_parser = RegularParagraphParser()
    mission = Mission(sql_db, "regular", regular_terms, regular_parser, scraper)
    mission.run(delay=0, thread_num=4)
